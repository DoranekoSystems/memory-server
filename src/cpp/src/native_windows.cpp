...
extern "C" SSIZE_T read_memory_native(int pid, uint64_t address, uint32_t size, unsigned char *buffer) {
...
extern "C" void enumerate_regions_to_buffer(DWORD pid, char *buffer, size_t buffer_size) {
    ...
    while (VirtualQueryEx(processHandle, addr, &memInfo, sizeof(memInfo))) {
        const char *state = memInfo.State == MEM_COMMIT ? " " : memInfo.State == MEM_RESERVE ? "r" : " ";
        
        // The permissions are approximated as Linux-like permissions, but not exactly the same.
        char permissions[5] = "----";
        if (memInfo.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY)) permissions[2] = 'x';
        if (memInfo.Protect & (PAGE_READWRITE | PAGE_READONLY | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE)) permissions[0] = 'r';
        if (memInfo.Protect & (PAGE_READWRITE | PAGE_WRITECOPY | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY)) permissions[1] = 'w';
        ...
        int written = snprintf(buffer + offset, buffer_size - offset, "%p-%p %s %s %s %s\n",
                               addr, (unsigned char *)addr + memInfo.RegionSize, state, permissions, (memInfo.Type == MEM_MAPPED ? "p" : " "), mappedFileName);
