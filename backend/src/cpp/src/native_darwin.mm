...
typedef int (*PROC_REGIONFILENAME)(int pid, uint64_t address, void *buffer,
                                   uint32_t buffersize);
...
extern "C" void enumerate_regions_to_buffer(pid_t pid, char *buffer,
  ...
  while (true) {
    ...
    if (info.is_submap) {
      ...
      pos +=
          snprintf(buffer + pos, buffer_size - pos, "%llx-%llx %s\n",
...
extern "C" ProcessInfo *enumprocess_native(size_t *count) {
  ...
  do {
    length = 0;
    err = sysctl(const_cast<int *>(name), (sizeof(name) / sizeof(*name)) - 1,
  ...
  if (err == 0 && result != nullptr) {
    *count = length / sizeof(struct kinfo_proc);
    ProcessInfo *processes =
        static_cast<ProcessInfo *>(malloc(*count * sizeof(ProcessInfo)));
...
static std::uint64_t get_image_size_64(int pid,
...
static std::uint64_t get_image_size_32(int pid,
                                       mach_vm_address_t base_address) {
  mach_header header;
...
static std::uint64_t get_module_size(int pid, mach_vm_address_t address,
                                     bool *is_64bit) {
  std::uint32_t magic;
  ...
  if (magic == MH_MAGIC_64) {
    ...
    *is_64bit = false;
    ...
    if (read_memory_native(pid, address, sizeof(fat_header),
                           reinterpret_cast<unsigned char *>(&fatHeader)) <=
        0) {
    ...
    if (read_memory_native(pid, address + sizeof(fat_header),
                           ...
                           reinterpret_cast<unsigned char *>(archs.data())) <=
        0) {
...
extern "C" ModuleInfo *enummodule_native(pid_t pid, size_t *count) {
  ...
  if (task_info(task, TASK_DYLD_INFO, reinterpret_cast<task_info_t>(&dyld_info),
                &count_info) != KERN_SUCCESS) {
  ...
  if (read_memory_native(
          pid, dyld_info.all_image_info_addr, sizeof(dyld_all_image_infos),
  ...
  if (read_memory_native(
          pid, reinterpret_cast<mach_vm_address_t>(all_image_infos.infoArray),
  ...
  for (const auto &info : image_infos) {
    char fpath[PATH_MAX];
    if (read_memory_native(
            pid, reinterpret_cast<mach_vm_address_t>(info.imageFilePath),
            PATH_MAX, reinterpret_cast<unsigned char *>(fpath)) > 0) {
      ModuleInfo module;
      if (strlen(fpath) == 0 && proc_regionfilename != nullptr) {
        char buffer[PATH_MAX];
        int ret = proc_regionfilename(
            pid, reinterpret_cast<std::uint64_t>(info.imageLoadAddress), buffer,
            sizeof(buffer));
  ...
  *count = moduleList.size();
  ModuleInfo *result =
      static_cast<ModuleInfo *>(malloc(*count * sizeof(ModuleInfo)));
